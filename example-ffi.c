...
int main(int argc, char **argv) {
  ...
  scheme_define(&sc, sc.global_env, mk_symbol(&sc, "custom-print"), mk_foreign_func(&sc, custom_print_func));
  ...
  scheme_load_string(&sc, "(define (fun a b) (custom-print (* a b)) (+ a b))");
  
  pointer func = scheme_eval(&sc, mk_symbol(&sc, "fun"));
  pointer args = cons(&sc, mk_integer(&sc, 3), cons(&sc, mk_integer(&sc, 5), sc.NIL));
